...
bool CParticleSystem::initPractile()
{
	...
	for (idn = 0; idn <MAX_PARTICLES; ++idn)
	{
		...
		paticle[idn].zd = float((rand() % 50) - 25.0f) * 10.0f;
...
{
	paticle[n].xd = float((rand() % 50) - 26.0f) * 10.0f;
	paticle[n].yd = float((rand() % 50) - 25.0f) * 10.0f;
	paticle[n].zd = float((rand() % 50) - 25.0f) * 10.0f;
}
//改变粒子加速度
