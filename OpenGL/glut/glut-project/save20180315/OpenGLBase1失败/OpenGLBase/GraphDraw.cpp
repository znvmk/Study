...
void CGraphDraw::DrawSand()//显示随机山势
{
	...
	m_tgawr.zTexture2D(texture, 5);		// 设置BACK贴图左
...
void CGraphDraw::CreateSkyBox(int a, int wi, int he, int le)//显示天空
{
	...
	float z = -MAP - length / 2;	// 天空的位置z
									///////////////////////////////////////////////////////////////////////////////
	m_tgawr.zTexture2D(texture, 1);		// 设置BACK贴图左
	glEnable(GL_TEXTURE_2D);
	...
	m_tgawr.zTexture2D(texture, 0);		// 设置FRONT贴图右
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);			// 多组独立填充四边形
	...
	m_tgawr.zTexture2D(texture, 2);		// 设置LEFT贴图前
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);			// 多组独立填充四边形
	glTexCoord2f(1.0f, 1.0f); glVertex3f(x, y + height, z);
	...
	m_tgawr.zTexture2D(texture, 3);		// 设置RIGHT贴图后
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);			// 多组独立填充四边形		
