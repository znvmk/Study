...
COpenGL3DView::COpenGL3DView()
{
	...
	m_pMouseMove.x = m_pMouseMove.y = 0.0;
...
void COpenGL3DView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	m_oglf.m_lbuttondown = 0;


	m_oglf.m_keyright = m_oglf.m_keyleft = m_oglf.m_keydown = m_oglf.m_keyup = 0;
...
void COpenGL3DView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	float a , b ;
	a = (rect.bottom - rect.top)/2;
	b = (rect.right - rect.left)/2;
	
	if (m_oglf.m_lbuttondown)
	{
		
			//if (point.x>b)m_oglf.m_keyright = 1;
			//else m_oglf.m_keyright = 1;
		
		
			//if (point.y>a)m_oglf.m_keydown = 1;
			//else m_oglf.m_keyup = 1;
