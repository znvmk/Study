...
int DrawGLScene(GLvoid)
{
	...
	glLoadIdentity();   // 重置当前的模型观察矩阵  
						//添加绘图代码 
	//m_gp->Box(4, -4, -10);
	//m_gp->picter(4, -4, -10);					//显示组合图形
	//m_gp->airplane(0, 8, -50);					//组合飞机
	m_gp->DisplayScene();						//摄像机
	m_gp->DrawGround();						//篮色网格地面线
	m_gp->picter(MAP + 10, 0, -MAP);				//显示组合图形
	m_gp->airplane(MAP + 50, 15, -MAP);			//组合飞机
