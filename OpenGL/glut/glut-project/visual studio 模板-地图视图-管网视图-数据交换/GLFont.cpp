...
{ 	HDC hdc = wglGetCurrentDC();        //获取设置场景
	...
	glRasterPos2f(x/100, y/100);        //平面定位
		...
		const char* ctlpsz = NULL;
		ctlpsz = (CW2A)str;



		glCallLists(strlen(ctlpsz), GL_UNSIGNED_BYTE, str); //绘制显示列表
...
{	glDisable(GL_TEXTURE_2D);     // 关闭贴图
	...
	Printfc3d(_T("立体汉字"),hFont,z);// 调用3D文字显示
...
{ CBitmap bitmap;									//设备相关位图变量
  ...
  /*LPCTSTR转换为const char* */
  const char* ctlpszText=NULL;
  ctlpszText= (CW2A)lpszText;


  ::GetTextExtentPoint32(MDC,lpszText,strlen(ctlpszText),&size);//获取字符位图大小
  ...
  TextOut(MDC, 0, 0, lpszText, strlen(ctlpszText));	//输出文字到暂存MDC
