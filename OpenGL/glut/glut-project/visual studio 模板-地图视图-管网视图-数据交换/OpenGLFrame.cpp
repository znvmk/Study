...
void COpenGLFrame::DrawScene()
{
	...
	//m_gd.DisplayScene();						 // 摄像机
	//m_gd.DrawSand();							 // 显示随机山势
	//m_gd.CreateSkyBox(3, 6, 3, 6);				 // 显示天
	//m_gd.picter(MAP + 10, 0, -MAP);				//显示组合图形
...
void COpenGLFrame::MessageTransfer()
{
	m_gd.m_keyleft= m_keyleft;
	m_gd.m_keyright= m_keyright;
	m_gd.m_keydown= m_keydown;
	m_gd.m_keyup= m_keyup;
	m_gd.m_keyshift= m_keyshift;
	m_gd.m_keypagedown= m_keypagedown;
	m_gd.m_keypageup= m_keypageup;

	m_gd.m_nViewSelect = m_nViewSelect;
}