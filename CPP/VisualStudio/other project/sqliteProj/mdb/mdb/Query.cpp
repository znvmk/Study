	...
	m_database(NULL)
{
	// 创建数据库
	this->CreateDatabase();
	//csTable, _T("nId"), _T("encode"), _T("entry"), _T("classify"), _T("sort"), _T("settop")
	csTable = _T("TableFlyCode");
	csTableClassify = _T("");
}
...
void Query::CreateDatabase()
{
	...
	//	 _T("TableFlyCode"), _T("nId"), _T("encode"), Const::MAX_SysNAME_LEN, _T("uScore"));
	/*csSQL.Format(_T("create table if not exists Sys(\
		...
		sort int not null,\
		settop int)\
		"),  _T("TableFlyCode"),_T("nId"), _T("encode"), _T("entry"), _T("classify"), _T("sort"), _T("settop"));*/
...
void Query::SetData()
{
	...
	for (int count = 0; count < 10000; count++)
	{
		...
		//	csTable, id, csEncode, csEntry, csTableClassify, sort, settop);
		sqlite3_prepare16_v2(m_database, csSQL, -1, &stmt, NULL);
		sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
...
void Query::Insert(int id, CString encode, CString entry, int sort, int settop, CString classify)
{
	// 检查该用户是否在数据库中
	sqlite3_stmt* stmt = NULL;
	CString csSQL;
	...
	if (sqlite3_step(stmt) != SQLITE_DONE)	// 在数据库中
	{
		//if (m_nScore > sqlite3_column_int(stmt, 1))
		{
			csSQL.Format(_T("UPDATE %s set %s='%s',%s='%s',%s='%s',%s='%d',%s='%d' WHERE %s='%d'"),
				csTable, _T("encode"), encode, _T("entry"), entry, _T("classify"), classify,
	...
	{
		csSQL.Format(_T("INSERT INTO %s VALUES('%d','%s','%s','%s','%d','%d')"),
			csTable, id, encode, entry, classify, sort, settop);
		sqlite3_prepare16_v2(m_database, csSQL, -1, &stmt, NULL);
		sqlite3_step(stmt);
	}
	sqlite3_finalize(stmt);
...
/**
 * 获取数据库中某类所有信息。
 * @param SysInfoList  某类所有信息
 */
CString Query::SearchClassify(CString classify)
{
	sqlite3_stmt* stmt = NULL;
	CString csSQL;
	CString csRecord;
	...
	// 从数据库中查询所有用户信息
	csSQL.Format(_T("SELECT * FROM %s WHERE %s = '%s' ORDER BY %s DESC"),
		csTable, _T("classify"), classify, _T("nId"));
	sqlite3_prepare16_v2(m_database, csSQL, -1, &stmt, NULL);
...
/**
 * 获取数据库中所有信息。
 * @param SysInfoList  所有信息
 */
CString Query::SearchAllData()
{
	sqlite3_stmt* stmt = NULL;
	CString csSQL;
	CString csRecord;
