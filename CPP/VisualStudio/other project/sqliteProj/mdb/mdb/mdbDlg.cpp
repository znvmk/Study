...
void CmdbDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_ENCODE, m_editEncode);
	DDX_Control(pDX, IDC_EDIT_ENTRY, m_editEntry);
	DDX_Control(pDX, IDC_EDIT_SORT, m_editSort);
	DDX_Control(pDX, IDC_EDIT_SETTOP, m_editSetTop);
}
...
BOOL CmdbDlg::OnInitDialog()
{
	...
	m_iId = m_iSort = m_iSetTop = m_iClassify = 0;
	m_csId = m_csEncode = m_csEntry = m_csSort = m_csSort = m_csClassify = _T("");
	m_csRecordSet = _T("");
...
void CmdbDlg::OnBnClickedDisplay()
{
	...
	SetStaticContrlText(IDC_STATIC_TABLE, _T("小鹤词库"));
	m_csRecordSet = Query.SearchAllData();
	SetStaticContrlText(IDC_STATIC_INFORMATION, _T("查询成功"), ShowListContrl(m_csRecordSet));
	GetDlgItem(ID_DISPLAY)->EnableWindow(1);
	//Query.GetAllData();
...
{
	// TODO: 在此添加控件通知处理程序代码
	if (IDNO == MessageBox(TEXT("确认要重置数据吗？"), TEXT("温馨提示"), MB_YESNO | MB_ICONQUESTION))
		return;
	SetStaticContrlText(IDC_STATIC_INFORMATION, _T("删除数据中..."));
	Query.Truncate();
	m_csRecordSet = Query.SearchAllData();
	SetStaticContrlText(IDC_STATIC_INFORMATION, _T("已清空数据表"), ShowListContrl(m_csRecordSet));
	AfxMessageBox(_T("已清空数据表"));
	...
	Query.SetData();
	m_csRecordSet = Query.SearchAllData();
	SetStaticContrlText(IDC_STATIC_INFORMATION, _T("重置成功"), ShowListContrl(m_csRecordSet));
	GetDlgItem(ID_RESET)->EnableWindow(1);
}
...
void CmdbDlg::OnBnClickedAdd()
{
	...
	if (m_csId == _T("") || m_csEncode == _T("") || m_csEntry == _T("") || m_csSort == _T("") || m_csSort == _T(""))
	{
		AfxMessageBox(_T("为保持词条完整性，请填写所有项目"));
		GetDlgItem(ID_ADD)->EnableWindow(1);
	}
	else
	{
		Query.Insert(m_iId,m_csEncode, m_csEntry, m_iSort, m_iSetTop,m_csClassify);
		m_csRecordSet = Query.SearchAllData();
		SetStaticContrlText(IDC_STATIC_INFORMATION, _T("添加成功"), ShowListContrl(m_csRecordSet));
		GetDlgItem(ID_ADD)->EnableWindow(1);
	}
}
...
void CmdbDlg::OnBnClickedSearch()
{
	...
	if (m_csId == _T("") && m_csClassify == _T(""))
	{
		AfxMessageBox(_T("请填写所有词条编码或词库分类"));
		GetDlgItem(ID_ADD)->EnableWindow(1);
		return;
	}
	if (m_csId != _T(""))
	{
		...
		if (ShowListContrl(Query.Search(m_iId)))
		{
			m_csRecordSet = Query.Search(m_iId);
			SetStaticContrlText(IDC_STATIC_INFORMATION, _T("查询成功"), ShowListContrl(m_csRecordSet));
		}
	...
	else if (m_csClassify != _T(""))
	{
		SetStaticContrlText(IDC_STATIC_INFORMATION, _T("查询数据中..."));
		GetDlgItem(ID_SEARCH)->EnableWindow(0);
		//Query.Insert(110, _T("na110"), 111);
		
		if (ShowListContrl(Query.SearchClassify(m_csClassify)))
		{
			m_csRecordSet = Query.SearchClassify(m_csClassify);
			SetStaticContrlText(IDC_STATIC_INFORMATION, _T("查询成功"), ShowListContrl(m_csRecordSet));
		}
...
void CmdbDlg::OnBnClickedExport()
{
	...
	//CString tabledata = Query.SearchAllData();
	CString tabledata = m_csRecordSet;
...
void CmdbDlg::getEditText()
{
	m_iId = m_iSetTop = m_iSetTop = 0;
	m_csId = m_csEncode = m_csEntry = m_csSort = m_csSort = _T("");
	m_editID.GetWindowTextW(m_csId);
	m_editEncode.GetWindowTextW(m_csEncode);
	m_editEntry.GetWindowTextW(m_csEntry);
	m_editSort.GetWindowTextW(m_csSort);
	m_editSetTop.GetWindowTextW(m_csSort);
	m_comboClassify.GetWindowTextW(m_csClassify);
	m_iClassify = m_comboClassify.GetCurSel();
	//csClassify.Format(_T("%d"), sel);
	//AfxMessageBox(csClassify);

	m_iId = _ttoi(m_csId);
	m_iSort = _ttoi(m_csSort);
	m_iSetTop = _ttoi(m_csSort);
