...
int main()
{
   ...
   // 为当前绑定的纹理对象设置环绕、过滤方式
    ...
    //变换
    //旋转:沿Z轴旋转90度
    //trans = glm::rotate(trans, glm::radians(gdp.angleZ), glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    // 上面这行代码就需要改为:
        ...
        //位移
        if (gdp.transKey)
            trans = glm::translate(trans, glm::vec3(gdp.transX, gdp.transY, gdp.transZ));
        //trans = glm::translate(trans, glm::vec3(-0.5, 0.1, 0.1));
        //*
        //旋转
        if (gdp.roateKey) {
            //旋转:沿X轴旋转
            trans = glm::rotate(trans, glm::radians(gdp.roateX), glm::vec3(1.0f, 0.0f, 0.0f));
            //旋转:沿Y轴旋转
            trans = glm::rotate(trans, glm::radians(gdp.roateY), glm::vec3(0.0f, 1.0f, 0.0f));
            //旋转:沿Z轴旋转
            trans = glm::rotate(trans, glm::radians(gdp.roateZ), glm::vec3(0.0f, 0.0f, 1.0f));
        }
        //放缩
        if (gdp.scaleKey)
            trans = glm::scale(trans, glm::vec3(gdp.scaleX, gdp.scaleY, gdp.scaleZ));
        // */
     //按获取的时间旋转
     //trans = glm::rotate(trans, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));
        ...
        /*
        trans = glm::translate(trans, glm::vec3(-1.2, 0.0, -2));
        trans = glm::rotate(trans, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans));
        float vertices[] = {
