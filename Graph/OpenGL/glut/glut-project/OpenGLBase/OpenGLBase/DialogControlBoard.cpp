...
BEGIN_MESSAGE_MAP(CDialogControlBoard, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON_ADD, &CDialogControlBoard::OnBnClickedButtonAdd)
	ON_BN_CLICKED(IDC_BUTTON_MINUS, &CDialogControlBoard::OnBnClickedButtonMinus)
...
void CDialogControlBoard::OnBnClickedButtonAdd()
{
	// TODO: 在此添加控件通知处理程序代码
	m_nNumber++;
...
void CDialogControlBoard::OnBnClickedButtonMinus()
{
	// TODO: 在此添加控件通知处理程序代码
	if (m_nNumber > 0)
	{
...
void CDialogControlBoard::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	switch (nChar)
	{
	case VK_DOWN: OnBnClickedButtonAdd();break;
	case VK_UP:if (m_nNumber > 0)OnBnClickedButtonMinus();;break;
	}
	UpdateData();
